...
export const addObjective = async ({resumeId, body}) => {
  try {
    const response = await api.post(`/profile/${resumeId}/objective`, {
      objective: body
    });
    return response;
  } catch (error) {
...
export const getObjective = async ({resumeId}) => {
  try {
    const response = await api.get(`/profile/${resumeId}/objective`);
    return response;
  } catch (error) {
    console.log(
      'Get Objective Error:',
